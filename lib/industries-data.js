...
export const industries = [
...

  {
    ...
    intro: [
      'Almost nobody reads a coffee shop website. They check it. It is 7:12 in the morning, they are two minutes from the turn, and they want three facts: are you open, is the drive-thru open, where does the car go. A site that hides the hours under a full-bleed hero has failed.',
      'The visitors worth more per head are in no hurry at all — somebody choosing where to spend a Saturday, an office manager ordering forty coffees, a buyer who tasted your beans and wants a price list. The site has to be quick for the first person and generous for the rest, then connect to the parts that earn while the counter is shut.',
    ],

    problemsIntro:
      'Four things a cafe build has to get right that a restaurant build does not. Swap "coffee shop" for "bistro" in any of them and it stops being true.',

    problems: [
      {
        title: 'Your hours are the most-read thing you will ever publish',
        body:
          'Opening hours out-read the menu, the story and the photographs put together, and they are the content most likely to be wrong. Cafes keep awkward schedules — early opens, late weekend closes, a kitchen that stops before the counter does, holidays nobody remembers to post. Within a month of launch the website, the map listing and the sign on the door disagree. Hours should be one record the site renders, and "open now" computed rather than typed.',
      },
      {
        title: 'The menu changes faster than anyone wants to file a ticket',
        body:
          'A seasonal drink lands on Tuesday, the pistachio syrup runs out on Thursday, the pastries are gone by ten. If changing any of that means emailing an agency, the menu is a historical document within a fortnight and staff start telling customers to ignore it — worse than having no menu online. Editing has to be something a manager does on a phone, with items hidden rather than deleted, because next season they return.',
      },
      {
        title: 'Ordering is a pickup problem, and pickup has a clock',
        body:
          'A latte does not travel. First-party cafe ordering is order-ahead for pickup, and it lives or dies on timing: how far ahead somebody can order, whether the drive-thru gets its own queue, what happens to a 6:55 order when you open at 7, whether the flow closes itself when the counter does. Restaurant checkouts, built for delivery windows and table bookings, get all four wrong — and the customer finds out while standing in your shop.',
      },
      {
        title: 'The bag business and the cafe business share a website and nothing else',
        body:
          'Retail bags, wholesale accounts and the cafe are three products with three buyers. Retail wants a grind option and a shipping cost. Wholesale wants a price list, a lead time and an account, and will not touch a consumer cart. Bolting a shop onto the menu page produces a checkout bad at both — these are separate paths from the navigation down.',
      },
    ],

    demosIntro:
      'Running demo sites, not screenshots — open any of them on a phone, which is where the real thing gets used. The note under each is our read on the kind of cafe it suits.',

    demoNotes: {
      draft1:
        'The safe default for a neighbourhood cafe that also sells beans — the editorial layout houses the origin story without pushing hours and menu below the fold.',
      draft2:
        'For a third-wave shop whose customers already know what a washed Ethiopian is. It filters out as many people as it attracts, which is sometimes the point.',
      draft3:
        'Look here if the coffee has a country attached to it — Yemeni, Ethiopian, Turkish, Levantine. The palette carries origin without falling back on a flag.',
      draft4:
        'For a cafe that keeps late hours and behaves like a lounge after eight — the only one of the five that makes an eleven-at-night dessert menu look intentional.',
      draft5:
        'Bright, hand-drawn and unintimidating — the easiest of the five to hang a loyalty scheme off. Suits a suburban shop with pushchairs in the doorway.',
    },

    softwareIntro:
      'A cafe that has outgrown a paper punch card and a shared spreadsheet usually needs two or three of these, and they scope better together.',

    serviceSlugs: ['ecommerce', 'loyalty-programs', 'booking-scheduling', 'reviews-reputation'],

    serviceNotes: {
      ecommerce:
        'Beans and merchandise with grind options and real shipping, kept apart from the cafe menu — plus wholesale, which needs accounts and a price list, not a cart.',
      'loyalty-programs':
        'The punch card, attached to a customer record instead of cardboard that lives in a wallet until it is lost. The tenth coffee is cheap marketing.',
      'booking-scheduling':
        'Catering and private hire taken as real requests with a date, a headcount and a lead time, rather than an email asking whether you do catering.',
      'reviews-reputation':
        'A review request that goes out after a visit rather than never. Cafe choice is a map decision, and on a map you are a rating and one photograph.',
    },
    ...
    caseStudy: {
      ...
      body: [
        'Yafa Golden Coffee is a Yemeni coffee house in Noblesville, Indiana, and the cafe client this page is written from. The site carries the whole operation rather than a menu photograph: the drinks, from a pistachio cream latte to Aden chai, the pastry counter, and single-origin beans from Yafa itself, with the origin story told as the reason the coffee tastes as it does.',
        'Four audiences share one site. The regular gets the Prairie Lakes address, hours running to midnight on a weekday and one in the morning at the weekend, and order-ahead pickup with an in-store or drive-thru choice. Everyone else gets a path of their own — host and catering, wholesale, and franchise enquiries. Those three are why the site is not just a menu, and exactly what a cafe template has no idea exists.',
      ],
      resultsGap:
    ...
    faqs: [
      {
        ...
        a: 'The map listing wins the search; the website is what it links to, and it holds everything a map cannot — the full menu, beans you ship, catering, wholesale, hiring. It is also the only one of the two you own.',
      },
      {
        q: 'Can our staff update the menu and the hours themselves?',
        a: 'Yes, and if they cannot, the site is wrong by next month. Items, prices, specials, sold-out flags and hours belong in an editor a manager can use on a phone. A smaller site that stays accurate beats a larger one that quietly rots.',
      },
      {
        q: 'Should we take orders on our own site, or keep the ordering platform we already pay for?',
        a: 'Usually keep it. If a platform is working, the sensible build hands off to it cleanly rather than replacing a checkout you would then own forever. Where a first-party flow earns its place is loyalty and repeat custom — that is yours, and on a platform it is not.',
      },
      {
        q: 'Have you actually built for a coffee shop before?',
        a: 'One, named on this page: Yafa Golden Coffee USA in Noblesville, Indiana — a cafe with a bean programme, catering and wholesale behind it. It is live and you can open it. One real site you can inspect beats a wall of logos.',
      },
    ],
  },

  {
    ...
    intro: [
      'A quick-service website has one measurable job and a lot of decoration around it: turn somebody who is already hungry into an order, in under a minute, on a phone, one-handed. Everything that delays that — an intro animation, a paragraph about the founder, a menu that is a photograph of a menu — costs orders that would otherwise have been placed.',
      'The second job is quieter and worth more across a year. An order through a delivery app costs a commission, and something larger than the commission: the customer record ends up with the app rather than with you. The website is the only place a repeat customer belongs to the restaurant, which is why ordering, loyalty and review requests belong on it.',
    ],

    problemsIntro:
      'Four things that make a quick-service build different from a restaurant build. A bistro site can get all four wrong and still work, which is why bistro templates keep being sold here.',

    problems: [
      {
        title: 'Third-party delivery is rent, and the tenant keeps your customer',
        body:
          'Delivery platforms bring real volume and take a commission on every order, and the part that costs more than the percentage is the customer record: they have it and you do not. You cannot send a Tuesday offer or notice that somebody stopped coming. The apps are worth keeping for reach — but an order that could have been first-party pickup and was not is paid for twice, once in commission and once in a customer you never met.',
      },
      {
        title: 'The menu is a price list, and price lists move',
        body:
          'Combos, family packs, wings by the count, sides at fifty cents — a quick-service menu is dozens of priced rows and every one of them moves. A menu rendered as an image, or typed into a page a developer owns, is stale the first time chicken gets more expensive, and the result is an argument at the counter. Prices belong in structured content the owner edits, so the site and the till agree.',
      },
      {
        title: 'The modifiers are the order',
        body:
          'Heat level, no pickles, sauce on the side, swap the fries. Customisation is not a garnish on a quick-service order, it is most of what distinguishes one from the next, and it is where ordering flows break: options that do not price correctly, combinations the kitchen cannot make, a notes field standing in for real modifiers. Get it wrong and the failure lands on a member of staff at the pass, not on the website.',
      },
      {
        title: 'Hours are not one row per day, and ordering has to respect them',
        body:
          'Quick service runs split shifts, prep gaps and different weekend closes. A site storing one open and one close per day cannot describe a Friday that runs eleven to three and then half four to eleven. That matters, because an ordering flow still taking orders while the kitchen is shut is worse than no ordering at all: it produces a customer at a locked door holding a receipt.',
      },
    ],

    demosIntro:
      'Running demo sites rather than screenshots — open them on a phone, where a hungry person actually meets one. The note under each is our read on the operation it suits.',

    demoNotes: {
      design1:
        'The classic quick-service direction — loud, hungry, unsubtle. Best for burgers, wings and anything where an appetising photograph outworks a paragraph.',
      design2:
        'For the fast-casual end: bowls, salads, build-your-own. The calmer layout has room for the ingredient and nutrition detail this customer actually reads.',
      design3:
        'The pickup-first direction, and the one to look at if you are pulling orders back off the delivery apps — almost everything points at a single order button.',
      design4:
        'Suits a single-cuisine counter with a strong identity, where the food has a place attached to it and the branding sells as hard as the menu does.',
      design5:
        'The most restrained of the five. For a small counter with a short menu, where the pitch is one thing done properly rather than forty done quickly.',
    },

    softwareIntro:
      'Most quick-service owners need the first two before anything else, and the other two shortly after a second location appears.',

    serviceSlugs: ['ecommerce', 'loyalty-programs', 'inventory-management', 'reviews-reputation'],

    serviceNotes: {
      ecommerce:
        'First-party ordering: the menu, modifiers that price correctly, pickup timing tied to real opening hours, and a checkout that keeps the customer record yours.',
      'loyalty-programs':
        'The reason a first-party order beats an app order twice over — you know who ordered, so the second visit is something you prompt rather than hope for.',
      'inventory-management':
        'Counts on the things that run out, tied to the menu, so a sold-out item leaves the site instead of being refunded after the order lands.',
      'reviews-reputation':
        'Review requests sent automatically after an order. Quick service is chosen from a map on a phone, where recency counts as much as the average.',
    },
    ...
    caseStudy: {
      ...
      body: [
        'Big Birdz Hot Chicken is a Nashville-style hot chicken counter on Allisonville Road in Indianapolis, and the quick-service client this page is written from. The menu is the site: combos, family packs, wings by the count, catfish, sides and sauces, each priced, with a heat level — no spice through extra spicy — chosen as part of the order rather than in a footnote.',
        'Pickup ordering runs on their own site and accepts orders in advance, while delivery is handed to DoorDash and Uber Eats — a deliberate split: reach where the platforms are good at reach, the customer kept where it matters. Catering has a page of its own, and the hours carry a real Friday break in the middle of the afternoon, the kind of detail a template flattens.',
      ],
      resultsGap:
    ...
    faqs: [
      {
        ...
        a: 'Some, not all. Pickup is where you win: a customer who was always going to have it delivered mostly stays on the app, but one who drives over anyway has no reason to pay app pricing. That is the order worth chasing, with loyalty attached so the second is easier.',
      },
      {
        q: 'Can we keep the ordering system we already have?',
        a: 'Usually yes. If your ordering platform works, the sensible build hands off to it cleanly rather than replacing a checkout you would then own forever. The budget is better spent on the menu, the hours and loyalty, which is where the return is.',
      },
      {
        q: 'Who updates prices when they change?',
        a: 'You do, from an editor, without emailing anybody. Quick-service prices move too often for anything else, and a menu that disagrees with the till is an argument at the counter. We can do it instead, but the site should not depend on us.',
      },
      {
        q: 'Have you actually built for a fast food restaurant before?',
        a: 'One, named on this page: Big Birdz Hot Chicken in Indianapolis — menu, heat levels, family packs, catering and first-party pickup ordering. It is live and you can order from it. That is the whole of our quick-service track record.',
      },
    ],
  },

  {
    ...
    intro: [
      'A photography website is unusual to build, because the portfolio is both the entire sales pitch and the heaviest object on the page. Every instinct says show more, larger, full-bleed. Every measurement says a hero taking four seconds on a phone loses the visitor before the first image resolves. Most of the craft here is making very large photographs arrive quickly, which is engineering rather than art direction.',
      'The other half is what happens once somebody is convinced. An enquiry is a date question before it is a price question, and a form collecting only a name and a message throws away a round trip. Then the work runs on for weeks after the shoot — contracts, balances, galleries — usually across three tools and an email thread, when it could be one link.',
    ],

    problemsIntro:
      'Four things that make a photography build different from any other portfolio site. A designer can ignore all four; a studio cannot.',

    problems: [
      {
        title: 'The portfolio is the product and the performance problem at once',
        body:
          'Photographers are the one client group right to want enormous images, and the group punished hardest for having them. A gallery is dozens of high-resolution files, and a phone on a weak connection abandons before the second one paints. It is solvable — modern formats, sized variants, real lazy loading — but it has to be designed in rather than patched later. A slow portfolio reads as an unprofessional one, unfairly and immediately.',
      },
      {
        title: 'The first question is the date, not the price',
        body:
          'A wedding enquiry, a corporate shoot and a portrait session all begin with availability, and a form asking only for a name and a message guarantees an extra round trip to establish the one fact that decides everything else. Ask for the date, the location and the kind of work up front and a good share of enquiries answer themselves. Somebody unwilling to type a date was never close to booking.',
      },
      {
        title: 'The job carries on for weeks after the shutter closes',
        body:
          'Booking is the middle of the process, not the end. There is a contract, a deposit, a balance due before delivery, a gallery to hand over, licensing choices, and a client who wants to know where all of it is. Most studios run that across a signing tool, a transfer link, an invoice and a long email thread, and the chasing lands on the photographer. It is the part most suited to a client portal and the part least often built.',
      },
      {
        title: 'One studio is usually three businesses',
        body:
          'Weddings, commercial work and event coverage share a camera and little else. Different buyers, lead times, price shapes and proof: a couple wants a full day told as a story, a marketing manager wants three usable frames and a licence to publish them. One portfolio that mixes them serves neither, and the usual fix — a single page called "Work" — is why enquiries keep arriving asking for the wrong thing.',
      },
    ],

    demosIntro:
      'Running demo sites, not screenshots — scroll a gallery on a phone, because that is where the decision gets made. The note under each is our read on the studio it suits.',

    demoNotes: {
      design1:
        'The wedding and couples default — warm, romantic, built for the long scrolling story a full-day gallery needs. Start here if most enquiries come from engaged couples.',
      design2:
        'Quiet and restrained, and the one that gets out of the way of the photographs most completely. Suits natural-light portraiture and headshots, where the pitch is taste.',
      design3:
        'The commercial direction: fast, bold, structured around deliverables rather than mood. What you want in front of a marketing manager comparing studios on turnaround.',
      design4:
        'For a fine-art or print practice — a muted gallery feel, and the easiest of the five to hang print sizes, editions and licensing off without reading as a shop.',
      design5:
        'Dark, cinematic and the most opinionated of the set. For editorial and fashion work, where the site is expected to be a statement rather than a neutral frame.',
    },

    softwareIntro:
      'The website is usually the smaller half of what a studio needs. These are the parts that stop admin eating the week after every shoot.',

    serviceSlugs: ['booking-scheduling', 'client-portals', 'customer-management', 'reviews-reputation'],

    serviceNotes: {
      'booking-scheduling':
        'Enquiries that collect the date, the location and the kind of shoot up front, checked against real availability, with confirmations and reminders sent automatically.',
      'client-portals':
        'One private link per client for the contract, the invoice, the balance and the delivered gallery — instead of a signing tool, a transfer link and a lost email thread.',
      'customer-management':
        'One record per client with the shoots, the licences granted and what was delivered, so a repeat booking two years later does not start in an inbox search.',
      'reviews-reputation':
        'A review request sent after delivery, while the photographs are still new. Photography is bought on trust from strangers, and a recent review carries more of that than a portfolio can.',
    },
    ...
    caseStudy: {
      ...
      body: [
        'Yasin Studios is a photography and video studio working largely with nonprofits and community organisations, and the studio client this page is written from. The site is built around two offers rather than one undifferentiated portfolio: event coverage — corporate events, fundraisers, community gatherings and mosque events — and video production: brand films, promotional reels and social content.',
        'The booking side is the part a portfolio template would have missed. Enquiries arrive through a form asking for the preferred date and time, the location and the details of the shoot, and the client gets an emailed confirmation instead of silence. The work is shown as a mixed gallery of video and stills — the honest way to present a studio that sells both.',
      ],
      resultsGap:
    ...
    faqs: [
      {
        ...
        a: 'Yes, and often you should. Delivery tools are cheap and good, and replacing one is rarely the best use of a build budget. What is worth building is the layer around it: the enquiry that captures a date, the record of who booked what, and one link a client can return to.',
      },
      {
        q: 'How do I show a large portfolio without the site being slow?',
        a: 'By deciding what loads first and letting everything else wait: sized image variants, modern formats, a deliberately light first screen, real lazy loading below it. The constraint is genuine but it is not a trade against quality — a fast site can be full of very large photographs, it just does not send them all at once.',
      },
      {
        q: 'Should prices be on the site?',
        a: 'Your call, and we do not have a rule about it. A starting figure filters out enquiries you would have declined anyway, which is worth more to a solo photographer than to a studio with a sales team. What we would avoid is a page that promises pricing and then offers only a form.',
      },
      {
        q: 'Have you actually built for a photographer before?',
        a: 'One, named on this page: Yasin Studios, a photography and video studio working with nonprofits and community organisations — portfolio, two service lines and a booking form that asks for the date. It is live and you can open it. That is the extent of our track record here.',
      },
    ],
  },
];
