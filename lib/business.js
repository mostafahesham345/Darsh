...
/* The Client Call Guide produces a structured engagement object. It arrives
   from the contract form as a JSON string; normalize it to an object (or null). */
function parseEngagement(val) {
  if (!val) return null;
  if (typeof val === 'object') return val;
  try { return JSON.parse(val); } catch { return null; }
}

/* Per-section build breakdown on a project: who owns what + the tool/provider
   behind each part of the site. Drops fully-empty rows. */
function cleanSections(arr) {
  if (!Array.isArray(arr)) return [];
  return arr
    .map((s) => ({
      name: (s && s.name ? String(s.name) : '').trim(),
      tool: (s && s.tool ? String(s.tool) : '').trim(),
      owner: (s && s.owner ? String(s.owner) : '').trim(),
    }))
    .filter((s) => s.name || s.tool || s.owner);
}
...
export async function createInvoice(data) {
  ...
  const payload = {
    ...
    notes: data.notes || '',
    // Income billed through Ingenium — excluded from the income-tax estimate
    // (Ingenium issues a 1099 for it).
    viaIngenium: Boolean(data.viaIngenium),
    status: 'draft',
    paidDate: null,
    createdAt: nowIso(),
    updatedAt: nowIso(),
...
export async function createProject(data) {
  ...
  const payload = {
    ...
    budget: data.budget || '',
    // Who owns what + the tool/provider behind each section of the site.
    ownership: (data.ownership || '').trim(),
    sections: cleanSections(data.sections),
    createdAt: nowIso(),
    updatedAt: nowIso(),
  };
...
export async function createContract(data) {
  ...
  const payload = {
    ...
    // Structured engagement summary from the Client Call Guide wizard (or null).
    engagement: parseEngagement(data.engagement),
...
export async function updateContract(id, data) {
  ...
  const payload = {
    ...
    terminationClause: data.terminationClause || '',
    additionalTerms: data.additionalTerms || '',
    // Keep the engagement object; preserve the existing one if none submitted.
    engagement: parseEngagement(data.engagement) || existing?.engagement || null,
...
/* ------------------------------ QUOTES ------------------------------ */
/* A quote is a list of priced sections; the total is the sum of section prices. */
export function quoteTotal(quote) {
  return (quote.sections || []).reduce((s, sec) => s + (Number(sec.price) || 0), 0);
}

function cleanQuoteSections(arr) {
  if (!Array.isArray(arr)) return [];
  return arr
    .map((s) => ({
      title: (s && s.title ? String(s.title) : '').trim(),
      description: (s && s.description ? String(s.description) : '').trim(),
      price: Number(s && s.price) || 0,
    }))
    .filter((s) => s.title || s.description || s.price);
}

export async function listQuotes() {
  if (!isReady()) return [];
  const snaps = await getDocs(query(collection(db(), QUOTES), orderBy('createdAt', 'desc')));
  return snaps.docs.map((d) => ({ id: d.id, ...d.data() }));
}
...
export async function nextQuoteNumber() {
  if (!isReady()) return `QT-${Date.now()}`;
  const snaps = await getDocs(collection(db(), QUOTES));
  const year = new Date().getFullYear();
  let max = 0;
  snaps.forEach((d) => {
    const m = /^QT-(\d+)-(\d+)$/.exec(d.data().quoteNumber || '');
    if (m && Number(m[1]) === year) {
      const v = Number(m[2]);
      if (v > max) max = v;
    }
...
export async function createQuote(data) {
  if (!isReady()) throw new Error('Firebase not initialized.');
  const payload = {
    quoteNumber: data.quoteNumber || (await nextQuoteNumber()),
    clientId: data.clientId || null,
    clientName: data.clientName || '',
    clientCompany: data.clientCompany || '',
    clientEmail: data.clientEmail || '',
    title: (data.title || 'Project Quote').trim(),
    issueDate: data.issueDate || new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }),
    validUntil: data.validUntil || '',
    currency: data.currency || 'USD',
    sections: cleanQuoteSections(data.sections),
    notes: data.notes || '',
    status: 'draft',
    sentAt: null,
    createdAt: nowIso(),
...
export async function updateQuoteStatus(id, status, extras = {}) {
  if (!isReady()) throw new Error('Firebase not initialized.');
  await updateDoc(doc(db(), QUOTES, id), { status, updatedAt: nowIso(), ...extras });
}
